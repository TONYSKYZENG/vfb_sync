...
void hal_gpio_init(int pin, int mode) {
    if (mode == 1) { // Output
        gpio_regs[GPFSEL_REG(pin)] &= ~(7 << GPFSEL_SEL(pin));
        gpio_regs[GPFSEL_REG(pin)] |= (1 << GPFSEL_SEL(pin));
    }
}

void hal_gpio_write(int pin, int val) {
    if (val) gpio_regs[GPSET_REG(pin)] = (1 << (pin % 32));
    else     gpio_regs[GPCLR_REG(pin)] = (1 << (pin % 32));
}
...
int main() {
    ...
    gpio_regs = (uint32_t *)mmap(NULL, 4096, PROT_READ | PROT_WRITE, MAP_SHARED, mem_fd, GPIO_BASE);
    close(mem_fd);
    ...
    // 4. 初始化 GPIO 模式
    hal_gpio_init(my_lcd.dc_pin, 1);
    hal_gpio_init(my_lcd.rst_pin, 1);
